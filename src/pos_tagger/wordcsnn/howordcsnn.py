...
N_CAP = 5
N_SPECIAL_TAG = 2  # <s> and </s> for POS tag


# Higher Order POS tagger
class HOWordCSnnTagger(chainer.Chain):
    def __init__(self, dim_wembed, dim_fembed, dim_pembed, dim_hidden, n_vocab, n_suf, n_label, window_size, l_pos_order, r_pos_order, objct='crossent', alpha=None):
        dim_embed = (dim_wembed + 2 * dim_fembed) * window_size + (l_pos_order + r_pos_order) * dim_pembed
        super(WordCSnnTagger, self).__init__(
            ...
            pos2embed=L.EmbedID(n_label+N_SPECIAL_TAG, dim_pembed),
        ...
        self.dim_pembed = dim_pembed
        self.pos_order = l_pos_order + r_pos_order
    ...
    #     batch_xs, batch_sufs, batch_caps, batch_poss = batch_features
    ...
    #     posembeds = self.pos2embed(Variable(batch_poss))
    ...
    #     posembeds = F.reshape(posembeds, (batchsize, (self.pos_order * self.dim_pembed)))
    #     all_embeds = F.concat((xembeds, sufembeds, capembeds, posembeds))
